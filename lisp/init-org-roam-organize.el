;;; init-org-roam-organize.el --- Organize roam-node references -*- lexical-binding: t; coding: utf-8 -*-
;;; Commentary:
;;
...
(defvar org-roam-organize//variable-type-alist
  '(
...
(defun org-roam-organize--check-variables (root_dir alist)
  (if
      ...
      (
       ...
       (result_message 
        (concat
            "All org-roam-organize/* variables are as follow.\n"
            )
        )
       (add_to_result_message_
        (lambda 
          (var_name var_value var_expected_type type_p_)
          (setq result_message
		(concat
                 result_message
                 (format 
        ...
        (let*
          ...
          (cond
           (
            ...
            (unless
                (and (boundp var_name) (listp var_value))
           ...
           (
            ...
            (unless
                (and (boundp var_name) (stringp var_value))
              (setq result_bool nil)
              )
            )
           (
...
(defun org-roam-organize--hash-table-to-alist (hash_table)
  ...
  )  ;; nreverse to reverse the list back to original order
...
(defun org-roam-organize--count-nodes-with-given-tag-list (tag_list &optional hash_to_alist)
  ...
  (when
    ...
    (let* 
	(
         (tag_count (make-hash-table :test 'equal))
         (result
          (org-roam-db-query
           [
      ...
      (cl-loop 
       ...
       for tag    = (car pair)
        ...
        (cons 
          ...
          (let 
              (
               (linked-ht (make-hash-table :test 'equal))
               )
            (dolist 
              (x linked_nodes) 
...
(defun org-roam-organize-goto-map-of-maps ()
  ...
  (if
      org-roam-organize-mode
      (let* 
	  ((id org-roam-organize/id-of-top-moc))
      (if
          (not id)
          (message "The id is not be defined, please check the config. ")      
        (let 
            ((location (org-id-find id 'marker)))
          (if location
              (progn
		(org-id-goto id)
...
(defun org-roam-organize-headline-move (source_pos)
  ...
  (if
    (and
     org-roam-organize-mode
     ;; org-roam-organize/move-target-moc-file
     ;; org-roam-organize/move-target-directory
     ;; org-roam-organize/move-source-tag
     ;; org-roam-organize/move-target-tag
     ;; (stringp org-roam-organize/move-target-directory)
     ;; (stringp org-roam-organize/move-target-moc-file)
     ;; (stringp org-roam-organize/move-source-tag)
     ;; (stringp org-roam-organize/move-target-tag)
     ;; (booleanp org-roam-organize/move-target-directory-id-or-not)
     ;; (booleanp org-roam-organize/move-target-filename-id-or-not)
     )
      (let* 
	  (
           (source_buf (current-buffer)) ; 提取id和node信息
        (info 
         (org-roam-organize--get-node-info-from-cite-in-headline source_pos)
         )
	...
	(unless 
      ...
      (save-excursion
        ...
        (let 
            ((subtree-str (org-copy-subtree t)))
          (org-cut-subtree)
          (with-current-buffer 
              (find-file-noselect org-roam-organize/move-target-moc-file)
            (goto-char (point-max))
            (insert subtree-str)
            )
          )
...
(defun org-roam-organize-headline-delete (&optional pos)
  ...
  (if
      ...
      (let* 
	  (
           (info (org-roam-organize--get-node-info-from-cite-in-headline pos))
           (id (plist-get info :id))
           (node (plist-get info :node))
           (tags (org-roam-node-tags node))
...
(defun org-roam-organize-update-mocs ()
  ...
  (if
      ...
      (let
	  (
           (tag_id org-roam-organize/tag-id-alist)
           (sth_unexpected nil)
	   )
	;; 开始提示
	...
	(cond
		...
		(entry tag_id)
			...
			(goto-char marker)
                      (let
                          (
                           (field (format "NUM_OF_%s_NODES" (upcase tag)))
                           )
...
(add-hook 'org-roam-organize-mode-hook
	  (lambda () 
		...
		(
		 (root_dir 
		  (when 
		      (boundp 'org-roam-organize/directory) 
		    org-roam-organize/directory
		    )
		  )
		 (check_result
		  (when 
		      (boundp 'org-roam-organize//variable-type-alist)
...
(global-set-key (kbd "C-c o c") 'org-roam-organize-check-variables)

(provide 'init-org-roam-organize)
;;; init-org-roam-organize.el ends here
